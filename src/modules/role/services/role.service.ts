...
export class RolesService {
  ...
  async create(createRoleDto: CreateRoleDto): Promise<Role> {
    const role = new Role();
    const nameExist = await this.roleRepository.findOneBy({name : createRoleDto.name})
    if(nameExist){
  ...
  async update(id:number,updateRoleDto : UpdateRoleDto):Promise<Role>{
    ...
    if(updateRoleDto.name){
      const nameExist = await this.roleRepository.findOneBy({ name: updateRoleDto.name });
